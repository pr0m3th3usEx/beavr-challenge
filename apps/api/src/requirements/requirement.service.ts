...
import { Document, DocumentStatus, RequirementWithStatus } from '@beavr/types';
...
export class RequirementService {
  ...
  async getAll(): Promise<RequirementWithStatus[]> {
    ...
    const requirementWithStatus: RequirementWithStatus[] = requirements.map(
      (requirement) => {
        const requiredDocs = requirement.requiredDocuments;

        // Group documents by docType
        const groupedDocs = requiredDocs.reduce<Record<string, Document[]>>(
          (acc, docType) => {
                ...
                ...documents
                  .filter(
                    (doc) =>
                      ...
                      doc.status !== 'EXPIRED', // We do not need expired documents
                  )
        ...
        const requiredDocumentsStatus = Object.entries(groupedDocs).reduce<
        ...
        return {
          ...requirement,
          status: Object.values(requiredDocumentsStatus).every(
            (status) => status === 'VALID',
          )
            ? 'COMPLIANT'
            : 'NOT_COMPLIANT',
